...
#define BAIDUBCE_MESSAGE_FORMAT "{\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"system\":\"Answer concisely, within 200 Chinese characters.\",\"stream\":false}"
#define BAIDUBCE_URL_FORMAT     "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-lite-8k?access_token=%s"

static bool baidu_token_configured(const char *token, const char *name)
{
    if (token == NULL || token[0] == '\0') {
        ESP_LOGE(TAG, "%s is empty. Configure it with idf.py menuconfig before using Baidu services.", name);
        return false;
    }
...
esp_err_t baidu_erniebot_send_request(const char *content)
{
    ...
    if (!baidu_token_configured(CONFIG_BAIDU_ERNIEBOT_ACCESS_TOKEN, "CONFIG_BAIDU_ERNIEBOT_ACCESS_TOKEN")) {
        return ESP_ERR_INVALID_STATE;
    }

    char *url = NULL;
    asprintf(&url, BAIDUBCE_URL_FORMAT, CONFIG_BAIDU_ERNIEBOT_ACCESS_TOKEN);
    if (url == NULL) {
...
esp_err_t baidu_asr_send_audio(const int16_t *audio, size_t len, size_t total_len)
{
    if (!g_baidu_asr_http_handle) {
        if (!baidu_token_configured(CONFIG_BAIDU_VOICE_ACCESS_TOKEN, "CONFIG_BAIDU_VOICE_ACCESS_TOKEN")) {
            return ESP_ERR_INVALID_STATE;
        }

        char *url = NULL;
        asprintf(&url,
                 "http://vop.baidu.com/server_api?dev_pid=1537&cuid=%s&token=%s",
                 CONFIG_BAIDU_ASR_CUID,
                 CONFIG_BAIDU_VOICE_ACCESS_TOKEN);
        if (url == NULL) {
            return ESP_ERR_NO_MEM;
        }
...
esp_err_t baidu_tts_send_text(const char *text)
{
    ...
    #define BAIDU_TTS_FORMAT "tex=%s&tok=%s&cuid=%s&ctp=1&lan=zh&spd=6&pit=5&vol=8&per=4100&aue=6&="

    char *payload = NULL;
    // Create JSON payload with model, max tokens, and content
    asprintf(&payload, BAIDU_TTS_FORMAT, text, CONFIG_BAIDU_VOICE_ACCESS_TOKEN, CONFIG_BAIDU_TTS_CUID);
...
bool baidu_tts_recv_audio(uint8_t **data, size_t *len, size_t *total_len, bool recv)
{
    ...
    if (recv) {
        ...
        int audio_length = 16000 * 2 * 80 / 1000;
