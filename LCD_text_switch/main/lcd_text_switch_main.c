/*
 * ESP-SparkBot LCD text switch demo.
 *
 * This project keeps the LCD initialization from LCD_blink, then redraws the
 * panel with either "HelloWorld" or "你好世界" on each timer tick.
 */
...
#ifndef CONFIG_LCD_TEXT_SWITCH_PERIOD_MS
#define CONFIG_LCD_TEXT_SWITCH_PERIOD_MS 1000
#endif
...
/*
 * SparkBot's official examples render Chinese through LVGL font files generated
 * from a real font. This demo keeps the direct esp_lcd path, but uses the same
 * idea: these four glyphs were pregenerated from a Chinese font instead of
 * hand-drawn.
 */
...
static void draw_chinese_text_to_strip(int strip_y, int strip_lines, int x, int y, uint16_t color)
{
    const int step = CHINESE_GLYPH_SIZE + CHINESE_GLYPH_GAP;

    for (int glyph = 0; glyph < CHINESE_GLYPH_COUNT; glyph++) {
        draw_chinese_glyph_to_strip(strip_y, strip_lines, glyph, x + glyph * step, y, color);
    }
}

static void lcd_draw_text_screen(bool show_chinese)
{
    ...
    for (int y = 0; y < LCD_V_RES; y += LCD_FLUSH_LINES) {
        ...
        if (show_chinese) {
            const int text_width = CHINESE_GLYPH_COUNT * CHINESE_GLYPH_SIZE +
                                   (CHINESE_GLYPH_COUNT - 1) * CHINESE_GLYPH_GAP;
            const int text_height = CHINESE_GLYPH_SIZE;
            const int text_x = (LCD_H_RES - text_width) / 2;
            const int text_y = (LCD_V_RES - text_height) / 2;
            draw_chinese_text_to_strip(y, lines, text_x, text_y, fg);
        } else {
            const char *text = "HelloWorld";
            const int text_width = ascii_text_width(text, ASCII_SCALE);
            const int text_height = ASCII_GLYPH_H * ASCII_SCALE;
            const int text_x = (LCD_H_RES - text_width) / 2;
            const int text_y = (LCD_V_RES - text_height) / 2;
            draw_ascii_text_to_strip(y, lines, text, text_x, text_y, ASCII_SCALE, fg);
        }
...
void app_main(void)
{
    ESP_LOGI(TAG, "LCD text switch started");
    lcd_init();

    while (1) {
        ESP_LOGI(TAG, "Show HelloWorld");
        lcd_draw_text_screen(false);
        vTaskDelay(pdMS_TO_TICKS(CONFIG_LCD_TEXT_SWITCH_PERIOD_MS));

        ESP_LOGI(TAG, "Show Chinese text");
        lcd_draw_text_screen(true);
        vTaskDelay(pdMS_TO_TICKS(CONFIG_LCD_TEXT_SWITCH_PERIOD_MS));
    }
}
