...
void power_adc_init(void)
{
    ...
    //-------------ADC Battery Estimation Init---------------//
    adc_battery_estimation_t adc_battery_config = {
        .external = {
            .adc_handle = power_adc_handle,
            .adc_cali_handle = power_adc_cali_handle,
        },
        .adc_channel = POWER_ADC_CHANNEL,
        .lower_resistor = POWER_LOWER_RESISTOR,
        .upper_resistor = POWER_UPPER_RESISTOR,
    };
    adc_battery_estimation_handle = adc_battery_estimation_create(&adc_battery_config);
    if (adc_battery_estimation_handle == NULL) {
        ESP_LOGE(TAG, "adc_battery_estimation_create failed");
    }
}
...
int get_power_value(void)
{
    ...
    if(adc_battery_estimation_handle) {
        ESP_ERROR_CHECK(adc_battery_estimation_get_capacity(adc_battery_estimation_handle, &power_value));
