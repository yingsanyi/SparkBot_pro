/*
 * SPDX-FileCopyrightText: 2023 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Unlicense OR CC0-1.0
 */

#include "app_weather.h"

#include "lwip/err.h"
#include "lwip/sys.h"
#include "esp_http_client.h"

#include "zlib.h"
#include "zutil.h"
#include "inftrees.h"
#include "inflate.h"

static const char *TAG = "app_weather";

#define LOG_TRACE(...)  ESP_LOGI(TAG, ##__VA_ARGS__)
#define CHECK(a, str, ret_val) do { \
        if (!(a)) { \
            ESP_LOGE(TAG,"%s(%d): %s", __FUNCTION__, __LINE__, str); \
            return (ret_val); \
        } \
    } while(0)

#define MAX_HTTP_RECV_BUFFER            (1024)
/**
 * @brief User Agent string
 *
 */
#define _UA_                            "esp32_S3_86_box"
/**
 * @brief key string, get it at https://console.qweather.com/#/apps
 *
 */
#define QWEATHER_REQUEST_KEY_PLACEHOLDER "key"
#define QWEATHER_API_HOST_PLACEHOLDER    "your_api_host"

#define _KEY_VALUE_                     CONFIG_QWEATHER_REQUEST_KEY

/**
 * @brief HTTPS request param. You can found location code at QWeather API page.
 *      And please keep gzip=n, otherwise you need to unzip the data by your self and use more Flash and RAM.
 *
 */
#define LOCATION_SHANGHAI               "101020100"
#define LOCATION_BEIJING                "101010100"
#define LOCATION_SHENZHEN               "101280601"

#define _OPTION_                        "location=101020100&lang=zh&unit=m"
#define _OPTION_MULTI                   "location=%s&lang=zh&unit=m"

/**
 * @brief See more at https://dev.qweather.com/docs/api/
 *
 */
#define WEATHER_SERVER      CONFIG_QWEATHER_API_HOST
#define WEB_URL_NOW         "https://" WEATHER_SERVER "/v7/weather/now?" _OPTION_MULTI
#define WEB_URL_AIR         "https://" WEATHER_SERVER "/v7/air/now?"     _OPTION_

static esp_err_t app_weather_parse_now(char *buffer, location_num_t location);

static air_info_t *air_info = NULL;
static weather_info_t *weather_info[LOCATION_NUM_MAX];

char weather_temp[WEATHER_TEMP_MAX_LEN] = "20\xC2\xB0";
char weather_text[WEATHER_TEXT_MAX_LEN] = "--";
char weather_icon[WEATHER_ICON_MAX_LEN] = "104";

static bool app_weather_key_is_valid(void)
{
    return CONFIG_QWEATHER_REQUEST_KEY[0] != '\0' &&
           strcmp(CONFIG_QWEATHER_REQUEST_KEY, QWEATHER_REQUEST_KEY_PLACEHOLDER) != 0;
}

static bool app_weather_host_is_valid(void)
{
    return CONFIG_QWEATHER_API_HOST[0] != '\0' &&
           strcmp(CONFIG_QWEATHER_API_HOST, QWEATHER_API_HOST_PLACEHOLDER) != 0;
}

static bool app_weather_response_is_json(const char *buffer)
{
    if (buffer == NULL) {
        return false;
    }

    while (*buffer == ' ' || *buffer == '\r' || *buffer == '\n' || *buffer == '\t') {
        buffer++;
    }
    return *buffer == '{';
}

static void app_weather_set_default_text(void)
{
    snprintf(weather_temp, sizeof(weather_temp), "20\xC2\xB0");
    snprintf(weather_text, sizeof(weather_text), "--");
    snprintf(weather_icon, sizeof(weather_icon), "104");
}

static int network_gzip_decompress(void *in_buf, size_t in_size, void *out_buf, size_t *out_size, size_t out_buf_size)
{
    int err = 0;
    z_stream d_stream = {0}; /* decompression stream */
    d_stream.zalloc = NULL;
    d_stream.zfree = NULL;
    d_stream.opaque = NULL;
    d_stream.next_in  = in_buf;
    d_stream.avail_in = 0;
    d_stream.next_out = out_buf;

    if ((err = inflateInit2(&d_stream, 47)) != Z_OK) {
        return err;
    }
    while (d_stream.total_out < out_buf_size - 1 && d_stream.total_in < in_size) {
        d_stream.avail_in = d_stream.avail_out = 1;
        if ((err = inflate(&d_stream, Z_NO_FLUSH)) == Z_STREAM_END) {
            break;
        }
        if (err != Z_OK) {
            inflateEnd(&d_stream);
            return err;
        }
    }

    if ((err = inflateEnd(&d_stream)) != Z_OK) {
        return err;
    }

    *out_size = d_stream.total_out;
    ((char *)out_buf)[*out_size] = '\0';

    return Z_OK;
}

esp_err_t response_handler(esp_http_client_event_t *evt)
{
    static char *data = NULL;
    static int data_len = 0;

    switch (evt->event_id) {
    case HTTP_EVENT_ERROR:
        printf("HTTP_EVENT_ERROR\n");
        break;

    case HTTP_EVENT_ON_CONNECTED:
        data_len = 0;
        break;

    case HTTP_EVENT_HEADER_SENT:
        break;

    case HTTP_EVENT_ON_HEADER:
        if (evt->data_len) {
            printf("%.*s", evt->data_len, (char *)evt->data);
        }
        break;

    case HTTP_EVENT_ON_DATA: {
        char *new_data = heap_caps_realloc(data, data_len + evt->data_len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (new_data == NULL) {
            printf("data realloc failed\n");
            heap_caps_free(data);
            data = NULL;
            data_len = 0;
            break;
        }
        data = new_data;
        memcpy(data + data_len, (char *)evt->data, evt->data_len);
        data_len += evt->data_len;
        data[data_len] = '\0';
        break;
    }

    case HTTP_EVENT_ON_FINISH: {
        size_t out_size = 0;
        //only show one city weather
        location_num_t location = 0;
        if (data == NULL || data_len == 0) {
            ESP_LOGW(TAG, "Weather response is empty");
            break;
        }
        if (app_weather_response_is_json(data)) {
            app_weather_parse_now(data, location);
            break;
        }

        size_t decode_maxlen = data_len * 4 + 1;
        if (decode_maxlen < 1024) {
            decode_maxlen = 1024;
        }
        char *decode_out = heap_caps_malloc(decode_maxlen, MALLOC_CAP_SPIRAM);
        if (NULL == decode_out) {
            printf("Failed allocate mem\n");
            break;
        }
        int zret = network_gzip_decompress(data, data_len, decode_out, &out_size, decode_maxlen);
        if (zret == Z_OK && out_size > 0) {
            // printf("HTTP_EVENT_ON_FINISH, location:%d, out_size:%d, %s\n", location, out_size, decode_out);
            app_weather_parse_now(decode_out, location);
        } else {
            snprintf(weather_temp, sizeof(weather_temp), "--");
            snprintf(weather_text, sizeof(weather_text), "HTTP ERR");
            snprintf(weather_icon, sizeof(weather_icon), "999");
            ESP_LOGW(TAG, "Weather response is neither JSON nor gzip/deflate payload, zlib=%d, first bytes=%.*s",
                     zret, 80, data);
        }

        if (decode_out) {
            free(decode_out);
            decode_out = NULL;
        }
        break;
    }

    case HTTP_EVENT_DISCONNECTED:
        if (data) {
            free(data);
            data = NULL;
        }
        break;

    default:
        break;
    }

    return ESP_OK;
}

esp_err_t app_weather_request(char *data)
{
    CHECK(app_weather_key_is_valid() && app_weather_host_is_valid(),
          "Invalid QWeather key or API host, skip weather request",
          ESP_ERR_INVALID_ARG);
    CHECK(data && data[0], "Invalid weather location", ESP_ERR_INVALID_ARG);

    ESP_LOGI(TAG, "location(west longtitude, south latitude) = %s", data);
    char *url_request = heap_caps_malloc(200, MALLOC_CAP_SPIRAM);
    CHECK(url_request, "Failed allocate mem", ESP_ERR_NO_MEM);
    int url_len = snprintf(url_request, 200, WEB_URL_NOW, data);
    if (url_len < 0 || url_len >= 200) {
        heap_caps_free(url_request);
        ESP_LOGE(TAG, "QWeather request URL is too long");
        return ESP_ERR_INVALID_SIZE;
    }
    ESP_LOGI(TAG, "QWeather host: %s", WEATHER_SERVER);

    esp_http_client_config_t config = {
        .url = url_request,
        .method = HTTP_METHOD_GET,
        .event_handler = response_handler,
        .buffer_size = MAX_HTTP_RECV_BUFFER,
        .timeout_ms = 5000,
    };
    // Set the headers
    esp_http_client_handle_t client = esp_http_client_init(&config);
    if (client == NULL) {
        heap_caps_free(url_request);
        return ESP_ERR_NO_MEM;
    }
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_http_client_set_header(client, "Host", WEATHER_SERVER);
    esp_http_client_set_header(client, "User-Agent", _UA_);
    esp_http_client_set_header(client, "X-QW-Api-Key", _KEY_VALUE_);
    esp_http_client_set_header(client, "Accept-Encoding", "gzip");
    esp_http_client_set_header(client, "Cache-Control", "no-cache");
    esp_http_client_set_header(client, "Accept", "*/*");

    // Send the request
    esp_err_t err = esp_http_client_perform(client);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "HTTP POST request failed: %s\n", esp_err_to_name(err));
    }

    esp_http_client_cleanup(client);

    if (url_request) {
        heap_caps_free(url_request);
    }

    return ESP_OK;
}

esp_err_t app_weather_start(void)
{
    app_weather_set_default_text();
    if (!app_weather_key_is_valid() || !app_weather_host_is_valid()) {
        ESP_LOGW(TAG, "Invalid QWeather key or API host. Weather UI will use defaults until both are configured.");
    }

    for (int i = 0; i < LOCATION_NUM_MAX; i++) {
        if (NULL == weather_info[i]) {
            weather_info[i] = heap_caps_malloc(sizeof(weather_info_t), MALLOC_CAP_SPIRAM);
            CHECK(weather_info[i], "Failed allocate mem", ESP_ERR_NO_MEM);

            weather_info[i]->describe = heap_caps_malloc(WEATHER_TEXT_MAX_LEN, MALLOC_CAP_SPIRAM);
            CHECK(weather_info[i]->describe, "Failed allocate mem", ESP_ERR_NO_MEM);

            weather_info[i]->temp = 20;
            weather_info[i]->icon_code = 104;
            snprintf(weather_info[i]->describe, WEATHER_TEXT_MAX_LEN, "--");
        }
    }

    return ESP_OK;
}

static esp_err_t app_weather_parse_now(char *buffer, location_num_t location)
{
    CHECK(buffer && buffer[0], "Parsing NULL weather payload", ESP_ERR_INVALID_ARG);
    CHECK(location < LOCATION_NUM_MAX, "Invalid weather location index", ESP_ERR_INVALID_ARG);

    cJSON *json = cJSON_Parse(buffer);
    cJSON *json_now = NULL;

    if (NULL != json) {
        cJSON *json_code = cJSON_GetObjectItem(json, "code");
        if (cJSON_IsString(json_code) && strcmp(json_code->valuestring, "200") != 0) {
            snprintf(weather_temp, sizeof(weather_temp), "--");
            snprintf(weather_icon, sizeof(weather_icon), "999");
            snprintf(weather_text, sizeof(weather_text), "API %s", json_code->valuestring);
            ESP_LOGE(TAG, "QWeather API returned code=%s, payload=%.*s",
                     json_code->valuestring, 160, buffer);
            cJSON_Delete(json);
            return ESP_FAIL;
        }

        json_now = cJSON_GetObjectItem(json, "now");
        if (NULL != json_now && cJSON_IsObject(json_now)) {
            cJSON *json_item_temp = cJSON_GetObjectItem(json_now, "temp");
            cJSON *json_item_icon = cJSON_GetObjectItem(json_now, "icon");
            cJSON *json_item_text = cJSON_GetObjectItem(json_now, "text");

            if (!cJSON_IsString(json_item_temp) || !cJSON_IsString(json_item_icon) || !cJSON_IsString(json_item_text)) {
                ESP_LOGE(TAG, "Weather JSON misses temp/icon/text");
                cJSON_Delete(json);
                return ESP_FAIL;
            }

            if (weather_info[location] == NULL) {
                cJSON_Delete(json);
                return ESP_ERR_INVALID_STATE;
            }

            snprintf(weather_temp, sizeof(weather_temp), "%s\xC2\xB0", json_item_temp->valuestring);
            snprintf(weather_icon, sizeof(weather_icon), "%s", json_item_icon->valuestring);
            snprintf(weather_text, sizeof(weather_text), "%s", json_item_text->valuestring);

            ESP_LOGI(TAG, "Temp : [%s]", json_item_temp->valuestring);
            ESP_LOGI(TAG, "Icon : [%s]", json_item_icon->valuestring);
            ESP_LOGI(TAG, "Text : [%s]", json_item_text->valuestring);

            weather_info[location]->temp = atoi(json_item_temp->valuestring);
            weather_info[location]->icon_code = atoi(json_item_icon->valuestring);

            snprintf(weather_info[location]->describe, WEATHER_TEXT_MAX_LEN, "%s", json_item_text->valuestring);
        } else {
            snprintf(weather_temp, sizeof(weather_temp), "--");
            snprintf(weather_icon, sizeof(weather_icon), "999");
            snprintf(weather_text, sizeof(weather_text), "JSON ERR");
            ESP_LOGE(TAG, "Weather JSON misses 'now', payload=%.*s", 160, buffer);
            cJSON_Delete(json);
            return ESP_FAIL;
        }
        cJSON_Delete(json);
    } else {
        snprintf(weather_temp, sizeof(weather_temp), "--");
        snprintf(weather_icon, sizeof(weather_icon), "999");
        snprintf(weather_text, sizeof(weather_text), "JSON ERR");
        ESP_LOGE(TAG, "Weather response is not JSON, first bytes=%.*s", 80, buffer);
        return ESP_FAIL;
    }

    return ESP_OK;
}

esp_err_t app_weather_get_current_info(weather_info_t *info, location_num_t location)
{
    CHECK(info, "Parsing NULL of info", ESP_ERR_INVALID_ARG);
    CHECK(location < LOCATION_NUM_MAX, "Invalid weather location index", ESP_ERR_INVALID_ARG);
    if (weather_info[location]) {
        memcpy(info, weather_info[location], sizeof(weather_info_t));
        return ESP_OK;
    }

    return ESP_FAIL;
}

esp_err_t app_weather_get_air_info(air_info_t *info)
{
    if (NULL == air_info) {
        air_info = heap_caps_malloc(sizeof(air_info_t), MALLOC_CAP_SPIRAM);
        CHECK(air_info, "Failed allocate mem", ESP_ERR_NO_MEM);

        air_info->aqi = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->level = heap_caps_malloc(16, MALLOC_CAP_SPIRAM);
        air_info->co = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->no2 = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->o3 = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->pm2p5 = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->pm10 = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);
        air_info->so2 = heap_caps_malloc(8, MALLOC_CAP_SPIRAM);

        CHECK(air_info->aqi, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->level, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->co, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->no2, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->o3, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->pm2p5, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->pm10, "Failed allocate mem", ESP_ERR_NO_MEM);
        CHECK(air_info->so2, "Failed allocate mem", ESP_ERR_NO_MEM);

        strcpy(air_info->aqi, "18");
        strcpy(air_info->level, "Excellent");
        strcpy(air_info->co, "0.5");
        strcpy(air_info->no2, "12");
        strcpy(air_info->o3, "101");
        strcpy(air_info->pm2p5, "5");
        strcpy(air_info->pm10, "15");
        strcpy(air_info->so2, "3");
    }

    CHECK(info, "Parsing NULL of info", ESP_ERR_INVALID_ARG);

    memcpy(info, air_info, sizeof(air_info_t));

    return ESP_OK;
}
