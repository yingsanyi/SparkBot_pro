...
static bool s_power_amp_initialized = false;

esp_err_t sparkbot_audio_power_amp_set(bool enable)
{
    if (!s_power_amp_initialized) {
        const gpio_config_t amp_config = {
            .pin_bit_mask = 1ULL << SPARKBOT_POWER_AMP_IO,
        ...
        s_power_amp_initialized = true;
    }

    return gpio_set_level(SPARKBOT_POWER_AMP_IO, enable ? 1 : 0);
}
...
static const audio_codec_ctrl_if_t *create_i2c_ctrl_if(void)
{
...
static const audio_codec_if_t *create_es8311_codec(esp_codec_dec_work_mode_t mode)
{
    const audio_codec_gpio_if_t *gpio_if = audio_codec_new_gpio();
    ESP_ERROR_CHECK(gpio_if ? ESP_OK : ESP_ERR_NO_MEM);

    const audio_codec_ctrl_if_t *i2c_ctrl_if = create_i2c_ctrl_if();
    ...
    return es8311_codec_new(&es8311_cfg);
}
...
esp_codec_dev_handle_t sparkbot_audio_codec_speaker_init(void)
{
    ...
    const audio_codec_if_t *es8311_dev = create_es8311_codec(ESP_CODEC_DEV_TYPE_OUT);
...
esp_codec_dev_handle_t sparkbot_audio_codec_microphone_init(void)
{
    ...
    const audio_codec_if_t *es8311_dev = create_es8311_codec(ESP_CODEC_DEV_WORK_MODE_BOTH);
    ESP_ERROR_CHECK(es8311_dev ? ESP_OK : ESP_ERR_NO_MEM);
